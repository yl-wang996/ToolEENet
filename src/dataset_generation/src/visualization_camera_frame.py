...
class CamViewPublish():
    ...
    def publish(self, image, depth_img, points, colors, ee_poses, seg_img, ee_points=None, cam_pose=None, sec=0.5):
        print("publishing...")
        hz = 10
        rate = rospy.Rate(hz)
        cv_rgb_image = cv2.cvtColor(np.asarray(image), cv2.COLOR_RGB2BGR)
        self.pub_rgb_img.publish(self.bridge.cv2_to_imgmsg(cv_rgb_image))

        # publish the seg image
        self.pbu_seg_img.publish(self.bridge.cv2_to_imgmsg(seg_img))
        ...
        for _ in range(sec*hz):
            ...
            if ee_points is not None:
                for idx, ee_name in enumerate(ee_points.keys()):
                    marker = self.build_marker_msg(
                        trans=ee_points[ee_name],
                        marker_id=idx,
                        text=ee_name,
                        time_now=time_now,
                    )
...
if __name__ == '__main__':
    ...
    for idx in [9]:
        ...
        meta_name = os.path.join(data_root, cat_name, obj_name, meta_file_name)
        meta_util = MetaUtils(data_root=data_root,meta_name=meta_name)
        ee_head1_pcd, ee_head1_colors = meta_util.get_ee_pcd('head1')
        ee_grip_pcd, ee_grip_colors = meta_util.get_ee_pcd('grip')

        obj_depth = meta_util.get_depth_array()
        # nromalzie the depth to 0-1
        depth_min = np.min(np.unique(obj_depth)[1:])
        obj_depth[obj_depth == np.unique(obj_depth)[0]] = depth_min
        obj_depth = (obj_depth - np.min(obj_depth)) / (np.max(obj_depth) - np.min(obj_depth))
        # convert the depth to the rgb image
        obj_depth = (obj_depth * 255).astype(np.uint8)
        depth_img = np.repeat(obj_depth[:, :, np.newaxis], 3, axis=2)
        ...
        seg_img = meta_util.get_affordance_seg()
        seg_ids = np.unique(seg_img)
        seg_image = np.zeros((seg_img.shape[0], seg_img.shape[1], 3)).astype(np.uint8)
        # convert the segmetation image to the rgb image
        for id in seg_ids:
            if id == 0:
                continue
            seg_image[seg_img == id] = np.random.randint(0, 255, 3)

        sample_idx = downsample_points(obj_points, num=2048)
        obj_points = obj_points[sample_idx]
        obj_colors = obj_colors[sample_idx]
