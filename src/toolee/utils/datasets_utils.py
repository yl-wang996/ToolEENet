import numpy as np
import cv2
# get 2d coordinates from image size
...
def aug_bbox_DZI(hyper_params, bbox_xyxy, im_H, im_W):
    ...
    Returns:
        center: the bbox center of the augmented bbox, it has been randomly shifted
        scale: the side length of the augmented bbox, it has been randomly scaled
    """
    x1, y1, x2, y2 = bbox_xyxy.copy()
    cx = 0.5 * (x1 + x2)  # center x
    cy = 0.5 * (y1 + y2)  # center y
    bh = y2 - y1  # bbox height
    bw = x2 - x1  # bbox width
    if hyper_params['DZI_TYPE'].lower() == "uniform":
        scale_ratio = 1 + hyper_params['DZI_SCALE_RATIO'] * (2 * np.random.random_sample() - 1)  # (1,) range[1-0.25, 1+0.25]
        shift_ratio = hyper_params['DZI_SHIFT_RATIO'] * (2 * np.random.random_sample(2) - 1)  # (2,) range[-0.25, 0.25]
    ...
    # make sure the scale is not over the image size
...
def crop_resize_by_warp_affine(img, center, scale, output_size, rot=0, interpolation=cv2.INTER_LINEAR):
    """
        warp affine transformation, from one 2D coordinate to another 2D coordinate, linear transformation.
        Try to use this function to transform the image in bbox to a square image with self.img_size.
        with rotation=0 means not rotate the image, only zoom in/out and shift the image.
    ...
    # get the affine transformation matrix from image in bbox to square img with size self.img_size
    trans = get_affine_transform(center, scale, rot, output_size)
    # apply the above affine transformation matrix to the 2D coordinate map of the whole image
...
def get_affine_transform(center, scale, rot, output_size, shift=np.array([0, 0], dtype=np.float32), inv=False):
    ...
    # get the rotated src points
    src_dir = get_dir([0, src_w * -0.5], rot_rad)
    dst_dir = np.array([0, dst_w * -0.5], np.float32)
    
    # three coordinates pairs can determine a warp affine transformation
...
def get_dir(src_point, rot_rad):
    '''
    get the direction of the point after rotation
    :param src_point: the src points to be rotated
    :param rot_rad: the rotation angle in redius
    :return:
    '''
