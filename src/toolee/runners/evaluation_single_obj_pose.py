...
def load_pose_prior(cat_name, ee_name):
	...
	pose_path_list = [f for f in os.listdir(os.path.join(data_path, cat_name)) if f.endswith('_pose.txt') and ee_name in f]
	pose_all = []
...
def infer_pose(dataset='test', result_path=None):
	...
	for _, batch_sample in enumerate(tqdm(data_loader, desc=f'{dataset}_loader')):
		...
		for idx in range(batch_size):
			...
			if cfg.task_type == 'obj_pose':
				pred_obj_RT = pred_RT
				gt_obj_RT = batch_sample['gt_RTs'][idx]
				gt_ee_RT = batch_sample['ee_RTs'][idx]
			elif cfg.task_type == 'ee_pose':
				pred_ee_RT = pred_RT
				gt_ee_RT = batch_sample['gt_RTs'][idx]
			else:
				raise ValueError(f"Invalid task type: {cfg.task_type}")
...
def evaluate(result_path, eval_taregt='ee_pose'):
	...
	fw.close()
	out_dir = os.path.join(inference_res_dir, f"task_{cfg.task_type}", f"target_{eval_taregt}")
...
def apply_prior_to_pred_results(result_path):
	with open(os.path.join(result_path), 'rb') as f:
		pred_results = cPickle.load(f)
	f.close()
	
	for idx, pred_result in tqdm(enumerate(pred_results),total=len(pred_results)):
		if pred_result['pred_ee_RT'] is not None:
			continue
		obj_pose = pred_result['pred_obj_RT']
		cat_name = pred_result['cat_name']
		ee_name = pred_result['affrdn_ee_name'].split('_')[-1]
		pose_prior_RT = load_pose_prior(cat_name=cat_name, ee_name=ee_name)
		refined_obj_pose = apply_pose_prior(pose_prior_RT, obj_pose)
		pred_results[idx]['pred_ee_RT'] = refined_obj_pose
...
def main():
	...
	print('Applying pose prior')
	apply_prior_to_pred_results(result_path)
	
	print('Evaluating ...')
	evaluate(result_path=result_path, eval_taregt='ee_pose')
